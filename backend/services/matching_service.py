...
class MatchingService:
    ...
    def calculate_match(
        ...
        """
        # 1. Semantic Score
        semantic_score = self.embedding_service.compute_semantic_score(clean_resume_text, clean_jd_text)
    ...
    def generate_comparison_summary(self, a: dict, b: dict) -> str:
        ...
        if diff_exp > 0:
            summary_parts.append(f"In terms of tenure, {higher_exp['name']} offers more years of professional experience ({higher_exp['total_experience_years']} years) compared to {lower_exp['name']} ({lower_exp['total_experience_years']} years).")
        else:
            summary_parts.append(f"Both candidates have equal years of professional experience ({a['total_experience_years']} years).")
            
        summary_parts.append(f"Education: {a['name']} holds a {a['highest_education_level']} degree, while {b['name']} holds a {b['highest_education_level']} degree.")
        
        if common_skills:
            summary_parts.append(f"They share core skills including: {', '.join(list(common_skills)[:4]).title()}.")
        if unique_a:
            summary_parts.append(f"{a['name']} brings unique proficiency in: {', '.join(list(unique_a)[:3]).title()}.")
        if unique_b:
            summary_parts.append(f"{b['name']} brings unique proficiency in: {', '.join(list(unique_b)[:3]).title()}.")
            
        if diff_match < 5:
            summary_parts.append("Recommendation: Both candidates are highly competitive and within a close match margin. Suggest interviewing both to evaluate soft skills and culture fit.")
        else:
            summary_parts.append(f"Recommendation: Prioritize {higher_match['name']} due to the {diff_match}% match score advantage, especially if their unique skills align with critical role requirements.")
...
def calculate_match(*args, **kwargs):
    return get_matching_service().calculate_match(*args, **kwargs)

def generate_comparison_summary(*args, **kwargs):
    return get_matching_service().generate_comparison_summary(*args, **kwargs)
